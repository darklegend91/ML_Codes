...
# Merge attendance and marks data on student ID or name (assume 'student_id' column exists)
merged_data = pd.merge(attendance_data, marks_data, on='Name')
merged_data = pd.merge(merged_data, activity_data, how='left', on='activity_name')
...
# Features: attendance and marks
X = merged_data[['attendance', 'marks']]
y = merged_data['target_activity_marks']  # Replace with the target variable column name
...
sns.lineplot(data=merged_data, x='date', y='marks', hue='student_id')
...
# Save the data and results to a new Excel file
output_file = 'student_performance_analysis.xlsx'
merged_data['Predicted Marks'] = model.predict(X)
